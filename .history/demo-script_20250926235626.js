...
class CambiaDemo {
  ...
  async logisticsVerification() {
    ...
    try {
      // Use the same unlock key that was generated during escrow creation
      const unlockKey = `demo-key-${this.orderId}-${Date.now()}`;
      ...
      await this.success(`Unlock Key Verified: ${unlockKey}`);
