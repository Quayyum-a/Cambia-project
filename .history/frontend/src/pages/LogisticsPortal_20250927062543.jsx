                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <div>
                      ...
                      <span className="ml-2">{orderDetails._id}</span>
                    </div>
                    <div>
                      <span className="font-medium">Status:</span>
                      <span className={`ml-2 px-2 py-1 rounded-full text-xs ${
                        orderDetails.status === 'completed' ? 'bg-green-100 text-green-800' :
                        orderDetails.status === 'prepared' ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    <div>
                      ...
                      <span className="ml-2">₦{orderDetails.totalPrice}</span>
                    </div>
                    <div>
                      <span className="font-medium">Vendor:</span>
                      <span className="ml-2">{orderDetails.vendor?.name}</span>
                    </div>
                    <div>
                      <span className="font-medium">Customer:</span>
                      <span className="ml-2">{orderDetails.sender?.name}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                      ...
                      <ul className="ml-2 mt-1 space-y-1">
                        {orderDetails.items?.map((item, index) => (
                          <li key={index} className="text-sm">
                            {item.quantity} {item.unit} of {item.name} - ₦{item.price}
                          </li>
                        ))}
                      </ul>
                    </div>
                    
                    {orderDetails.proofOfPackaging && (
                      <div>
                        <span className="font-medium">Proof of Packaging:</span>
                        <div className="ml-2 mt-1">
                          <a 
                            href={`https://ipfs.io/ipfs/${orderDetails.proofOfPackaging}`}
                            target="_blank"
                    ...
                    {orderDetails.escrowId && (
                      <div>
                        <span className="font-medium">Escrow ID:</span>
                        <span className="ml-2 text-sm font-mono">{orderDetails.escrowId}</span>
                      </div>
                    )}
                  </div>
                </div>
