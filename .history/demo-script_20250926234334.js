...
class CambiaDemo {
  ...
  async browseProducts() {
    ...
    try {
      // Get products from demo vendor
      const response = await axios.get(`${API_BASE}/api/products/vendor/${this.vendorId}`);
      const products = response.data;

      await this.success(`Found ${products.length} products from vendor ${this.vendorId}`);
  ...
  // Step 3: Create Order
  async createOrder() {
    await this.log('Step 3: Order Creation', '📦');
    ...
    try {
      const orderData = {
        ...
        items: [
          {
            product: 'demo-product-1', // This would be actual product ID
            quantity: 2
          },
          {
            product: 'demo-product-2', // This would be actual product ID
            quantity: 1
          }
        ],
        meta: {
          orderDate: new Date().toISOString(),
          customerNotes: 'Demo order for investor presentation'
        }
      };
      ...
      await this.success(`Total Amount: ₦${response.data.totalPrice || 5000}`);

    } catch (err) {
      await this.error(`Order creation failed: ${err.response?.data?.error || err.message}`);
      return false;
    }
  ...
  async setupEscrow() {
    ...
    try {
      ...
      // Save escrow ID to order
      await axios.post(`${API_BASE}/api/orders/${this.orderId}/escrow`, {
  ...
  async vendorFulfillment() {
    await this.log('Step 5: Vendor Order Fulfillment', '🏪');

    try {
      // Vendor receives order
      await axios.post(`${API_BASE}/api/vendor/orders/${this.orderId}/receive`, {}, {
        headers: { Authorization: `Bearer ${this.token}` }
      });
      ...
      await axios.post(`${API_BASE}/api/vendor/orders/${this.orderId}/prepare`, {}, {
        headers: { Authorization: `Bearer ${this.token}` }
      });
      ...
      await this.success('Vendor uploaded proof of packaging');
  ...
  async logisticsVerification() {
    ...
    try {
      // Logistics verifies and releases payment
      const unlockKey = `demo-key-${this.orderId}`;
      ...
      await this.success(`Unlock Key: ${unlockKey}`);
