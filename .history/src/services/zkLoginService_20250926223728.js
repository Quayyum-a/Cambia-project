...
class ZkLoginService {
  constructor() {
    ...
    if (!this.salt) {
      throw new Error('ZKLOGIN_SALT environment variable is required');
    }
  }
  ...
  generateEphemeralKeypair() {
    try {
      ...
      // Convert salt to BigInt properly
      let saltBigInt;
      try {
        // Create a simple hash of the salt string and convert to BigInt
        const crypto = require('crypto');
        const hash = crypto.createHash('sha256').update(this.salt).digest('hex');
        saltBigInt = BigInt('0x' + hash.substring(0, 16));
      } catch (saltError) {
        // Fallback: use a default salt for testing
        console.warn('Using fallback salt for testing');
        saltBigInt = BigInt('0x1234567890abcdef');
      }

      // Generate nonce using the ephemeral public key
      ...
      throw new Error(`Ephemeral keypair generation failed: ${error.message}`);
    }
  }
