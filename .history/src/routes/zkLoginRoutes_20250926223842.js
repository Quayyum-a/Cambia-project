...
router.post('/complete', async (req, res) => {
  try {
    ...
    // For testing purposes, create a simplified proof structure
    const proof = {
      proof: {
        proofPoints: {
          a: ['0x1', '0x2'],
        ...
        issBase64Details: {
          value: Buffer.from(userInfo.iss || 'https://accounts.google.com').toString('base64'),
          indexMod4: 0
        },
        headerBase64: Buffer.from(JSON.stringify({ alg: 'RS256', typ: 'JWT' })).toString('base64')
      },
      extendedEphemeralPublicKey: sessionData.ephemeralPublicKey,
      userSalt: zkLoginService.salt,
      addressSeed: zkLoginService.generateAddressSeed(userInfo.sub)
    };
