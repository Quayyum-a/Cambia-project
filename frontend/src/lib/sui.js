...
/**
 * Hook for using Sui escrow service
 */
export function useSuiEscrow(suiClient) {
  const [escrowService, setEscrowService] = React.useState(null);
  const [loading, setLoading] = React.useState(true);

  React.useEffect(() => {
    getSuiConfig().then(config => {
      if (config.packageId && suiClient) {
        setEscrowService(new SuiEscrowService(suiClient, config.packageId));
      }
      setLoading(false);
    });
  }, [suiClient]);

  return { escrowService, loading };
}
